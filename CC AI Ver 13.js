...
function seasonChecker() {
    var count = 0;
    var bound;
    var seasonUpgrades = [16, 6, 19, 7, 7];
    if (seasonCounter == 0)
        bound = [152, 168];
    if (seasonCounter == 1)
        bound = [169, 174];
...
function trainDragon() {
    ...
    if (Game.UpgradesById[324].bought == 1 && Game.Objects["Javascript console"].amount >= 400){
	...
	for (var i in stocksBought){
		if (stocksBought[i][1] > 0){ //if I own stock.  logic to sell
			if (stocksBought[i][2] < stocksCurrent[i][2]){ //if bought rank is less than current rank
				...
				if (stocksBought[i][1] < stocksCurrent[i][1] && stocksCurrent[i][1] > marketAvg[i][stocksBought[i][2] - 1]){ //if bought value is less than current value
					stocksBought[i][4] = sellStock(i);
				}else{
...
function downloadStockMarketData() {
	...
	var hiddenElement = document.createElement('a');
	hiddenElement.href = 'SMdata:text/csv;charset=utf-8,' + encodeURI(csv);
	hiddenElement.target = '_blank';
